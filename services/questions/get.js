...
export async function getTriviaQuestions(userId, executeFinallyFnHandler, executeErrorHandlerFn, handleReqSuccessLogic) {
    try {
        const url = new URL(QUESTIONS_API_URL)
        url.searchParams.append('userId', userId)
        const response = IS_TESTING ? { status: 200, data: new Array(2).fill(TEST_QUESTIONS).flat() } : await axios.get(QUESTIONS_API_URL);
